...
export default function InboundSequenceEditorPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params)
  const isNew = id === 'new'
  ...
  useEffect(() => {
    ...
    if (!isNew) {
      Promise.all([
        fetch(`/api/inbound/sequences/${id}`).then(r => r.json()),
        fetch(`/api/inbound/sequences/${id}/steps`).then(r => r.json()),
      ]).then(([seq, stps]) => {
        setSequence(seq)
        setSteps(stps || [])
        setLoading(false)
  ...
  async function save() {
    ...
    try {
      ...
      if (isNew) {
        ...
        const res = await fetch(`/api/inbound/sequences/${id}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(sequence),
        })
      ...
      // Save steps
      await fetch(`/api/inbound/sequences/${seqId}/steps`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ steps }),
      })

      router.push('/inbound/sequences')
    } catch (err) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
      <div className="flex items-center gap-3">
        <Link href="/inbound/sequences" className="p-2 hover:bg-gray-800 rounded-lg transition-colors">
          <ArrowLeft className="w-4 h-4 text-gray-400" />
        </Link>
        <h1 className="text-xl font-semibold text-white flex-1">
          {isNew ? 'New Inbound Sequence' : `Edit: ${sequence.name}`}
        </h1>
        <button
          onClick={save}
          disabled={saving}
      ...
      {error && (
        <div className="bg-red-900/20 border border-red-700/50 text-red-400 text-sm rounded-lg px-4 py-3">
          {error}
        </div>
      )}
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              placeholder="e.g. Inbound Lead Nurture"
        ...
        <div>
          <label className="block text-xs text-gray-400 mb-1.5">
            AI System Prompt
            <span className="text-gray-600 ml-1">(context for &#123;&#123;ai: ...&#125;&#125; blocks)</span>
          </label>
          <textarea
            value={sequence.system_prompt}
            onChange={e => setSequence({ ...sequence, system_prompt: e.target.value })}
            rows={3}
            placeholder="e.g. You are Manuela at Darkroom, a performance marketing agency..."
        ...
        <div className="flex items-center gap-2">
          ...
          />
          <label htmlFor="is_active" className="text-sm text-gray-400">Active (enrolls new form submissions)</label>
        </div>
      </div>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
        ...
        {steps.length === 0 ? (
          <div className="p-12 text-center">
            <p className="text-gray-500 text-sm">No steps yet. Add your first step above.</p>
          </div>
        ) : (
          <div className="flex">
            {/* Step tabs */}
              ...
              {steps.map((step, idx) => (
                <button
                  ...
                  className={`w-full text-left px-4 py-3 border-b border-gray-800/50 transition-colors ${
                    activeStep === idx ? 'bg-indigo-900/30 text-indigo-300' : 'text-gray-400 hover:bg-gray-800/50 hover:text-gray-200'
                  }`}
                >
                  <p className="text-xs font-medium">Step {step.step_number}</p>
                  <p className="text-xs opacity-60 mt-0.5">
                    Day {step.day_offset} · {step.step_type}
                  </p>
                </button>
              ))}
            </div>
            ...
            {currentStep && (
              <div className="flex-1 p-5 space-y-4">
                  ...
                  <>
                    <div>
                      ...
                      <input
                        ...
                        placeholder="Subject line — use {{company_name}}, {{first_name}}, {{ai: ...}}"
                        className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-indigo-500"
                      />
                    </div>
                    <div>
                    ...
                    <p className="text-xs text-gray-600">
                      Merge fields: {'{{first_name}}'}, {'{{company_name}}'}, {'{{services_interested}}'}, {'{{media_budget}}'} · AI: {'{{ai: write a custom hook}}'}
                    </p>
                  </>
                )}
                ...
                {currentStep.step_type === 'linkedin' && (
                  <div>
                    <label className="block text-xs text-gray-400 mb-1.5">LinkedIn Message (manual — shown as reminder)</label>
                    <TemplateEditor
                      value={currentStep.body_template}
                      onChange={val => updateStep(activeStep, 'body_template', val)}
